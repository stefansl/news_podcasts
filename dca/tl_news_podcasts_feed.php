...
(
    ...
    'palettes' => array
    (
        'default' => '{title_legend},title,alias,language,format;{description_legend},subtitle,summary,description,category;{author_legend},owner,email,image,author,copyright;{archives_legend},archives;{config_legend},maxItems,feedBase'
    ),
    // Fields
    'fields'   => array
    (
        ...
        switch (Input::get( 'act' )) {
            ...
            case 'show':
                if (!in_array( Input::get( 'id' ),
                        $root ) || (Input::get( 'act' ) == 'delete' && !$this->User->hasAccess( 'delete', 'itunesfeedp' ))
                ) {
                    $this->log( 'Not enough permissions to ' . Input::get( 'act' ) . ' itunes feed ID "' . Input::get( 'id' ) . '"',
                        __METHOD__, TL_ERROR );
